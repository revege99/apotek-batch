<x-app-layout>
            ...
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <span class="shrink-0" x-data>
                    <button
                        ...
                        @click="$dispatch('open-create-unit-modal')"
                    >
    ...
    <div
        x-data="{
            ...
            editModalOpen: @js($editingUnit !== null),
            closeEdit() {
                this.editModalOpen = false;
                window.location = @js(route('master-data.satuan-obat'));
            },
        }"
        @open-create-unit-modal.window="createModalOpen = true"
                                ...
                                <td class="px-4 py-2.5 text-center align-middle">
                                    ...
                                    >
                                        ...
                                        <template x-teleport="body">
                                            ...
                                            >
                                            <a
                                                href="{{ route('master-data.satuan-obat', ['edit' => $item->id]) }}"
                                            ...
                                            <button
                                                ...
                                                @click="close(); openDeleteDialog(@js([
                                                    'action' => route('master-data.satuan-obat.destroy', $item),
                                                    'title' => 'Hapus master satuan obat ini?',
                                                    'description' => 'Master '.$item->name.' akan dihapus dari daftar satuan obat.',
                                                    'warning' => 'Pastikan master ini sudah tidak dipakai pada data obat atau referensi satuan lain.',
                                                    'name' => $item->name,
                                                    'code' => $item->code,
                ...
                <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                    ...
                    @if ($errors->any() && old('_modal') === 'create')
                        <div class="mt-5 rounded-[1.35rem] border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                            Periksa kembali input master satuan obat.
                        </div>
                    @endif
            ...
            >
                <div class="flex min-h-full items-start justify-center p-4 sm:items-center sm:p-6">
                    <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                        ...
                        <form method="POST" action="{{ route('master-data.satuan-obat.update', $editingUnit) }}" class="mt-6 space-y-4">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="space-y-2">
                                ...
                                >{{ old('description', $editingUnit->description) }}</textarea>
                            </div>
                            ...
                            <label class="flex items-start gap-3 rounded-[1.35rem] border border-slate-200/80 bg-slate-50/80 px-4 py-4">
                                <input
                                    ...
                                    name="is_active"
                                    value="1"
                                    @checked(old('is_active', $editingUnit->is_active))
