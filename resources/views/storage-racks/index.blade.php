<x-app-layout>
        ...
        <div class="space-y-4">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <span class="shrink-0" x-data>
                    <button type="button" class="ui-action-btn ui-action-btn--soft px-4" @click="$dispatch('open-create-rack-modal')">
    ...
    <div x-data="{ createModalOpen: @js($errors->any() && old('_modal') === 'create'), editModalOpen: @js($editingRack !== null), closeEdit(){ this.editModalOpen = false; window.location = @js(route('master-data.rak-obat')); } }" @open-create-rack-modal.window="createModalOpen = true" @keydown.escape.window="createModalOpen = false; if (editModalOpen) closeEdit()" class="space-y-5">
        <section x-data="masterDeleteDialog()" @keydown.escape.window="closeDeleteDialog()" class="panel-surface overflow-visible p-0">
            <div class="border-b border-slate-200/80 px-6 py-5">
                <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                    <div class="flex flex-wrap items-center gap-2.5">
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">{{ number_format($stats['total']) }} rak</span>
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">{{ number_format($stats['linked_locations']) }} lokasi dipakai</span>
                    ...
                    <form method="GET" action="{{ route('master-data.rak-obat') }}" class="flex flex-wrap items-center justify-end gap-2">
                        <div class="w-[200px] shrink-0"><label for="search" class="sr-only">Cari rak</label><input id="search" name="search" type="text" value="{{ $search }}" placeholder="Cari rak" class="ui-control" @input.debounce.350ms="$el.form?.requestSubmit()"></div>
                        <div class="w-[160px] shrink-0"><label for="storage_location_id" class="sr-only">Lokasi</label><select id="storage_location_id" name="storage_location_id" class="ui-select-control"><option value="">Semua lokasi</option>@foreach ($locations as $location)<option value="{{ $location->id }}" @selected($locationId === $location->id)>{{ $location->name }}</option>@endforeach</select></div>
            ...
            <div class="overflow-visible">
                <table class="master-table min-w-full divide-y divide-slate-200/80 text-[0.8rem]">
                    <thead><tr class="text-left text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-slate-400"><th class="px-4 py-2.5">Kode</th><th class="px-4 py-2.5">Nama Rak</th><th class="px-4 py-2.5">Lokasi</th><th class="px-4 py-2.5">Deskripsi</th><th class="px-4 py-2.5">Status</th><th class="table-action-head">Aksi</th></tr></thead>
                    <tbody class="divide-y divide-slate-200/80 bg-white">
                        @forelse ($items as $item)
                            <tr class="align-middle">
                                ...
                                <td class="px-4 py-2.5"><div class="flex min-h-8 items-center text-slate-700">{{ $item->location?->name ?: '-' }}</div></td>
                                ...
                                <td class="px-4 py-2.5 text-center align-middle">
                                    <div x-data="floatingActionMenu()" @keydown.escape.window="close()" @click.window="if (open && ! $refs.trigger.contains($event.target) && ! ($refs.panel && $refs.panel.contains($event.target))) close()" class="relative flex min-h-8 items-center justify-center">
                                        <button x-ref="trigger" type="button" class="inline-flex h-7 w-7 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-600 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700" title="Aksi rak" aria-label="Aksi {{ $item->name }}" @click="toggleMenu()">
                                        ...
                                        <template x-teleport="body">
                                            <div x-cloak x-show="open" x-ref="panel" x-transition.opacity.duration.120ms x-bind:style="menuStyles" class="fixed z-[70] w-40 overflow-hidden rounded-xl border border-slate-200 bg-white py-1.5 shadow-xl shadow-slate-200/70">
                                                <a href="{{ route('master-data.rak-obat', ['edit' => $item->id]) }}" class="flex items-center gap-2 px-3 py-2 text-[0.8rem] font-medium text-slate-700 transition hover:bg-slate-50 hover:text-emerald-700" @click="close()">
                                                ...
                                                <button type="button" class="flex w-full items-center gap-2 px-3 py-2 text-left text-[0.8rem] font-medium text-rose-700 transition hover:bg-rose-50 hover:text-rose-800" @click="close(); openDeleteDialog(@js(['action' => route('master-data.rak-obat.destroy', $item), 'title' => 'Hapus data rak ini?', 'description' => 'Rak '.$item->name.' akan dihapus dari master data penyimpanan.', 'warning' => 'Pastikan rak ini belum dipakai pada modul penempatan atau referensi rak lain.', 'name' => $item->name, 'code' => $item->code, 'confirm_label' => 'Ya, hapus rak']))">
                        ...
                        @empty
                            <tr><td colspan="6" class="px-5 py-14 text-center"><div class="mx-auto max-w-md space-y-3"><div class="empty-title">Belum ada data rak</div><p class="content-copy">Tambah rak pertama, lalu hubungkan ke lokasi seperti gudang atau ruang apotik jika perlu.</p><button type="button" @click="createModalOpen = true" class="ui-action-btn ui-action-btn--soft">Input rak pertama</button></div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
        ...
        <div x-cloak x-show="createModalOpen" x-transition.opacity class="fixed inset-0 z-40 overflow-y-auto bg-slate-950/50 backdrop-blur-sm" @click.self="createModalOpen = false">
            <div class="flex min-h-full items-start justify-center p-4 sm:items-center sm:p-6">
                <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                    <div class="flex items-start justify-between gap-4"><div class="inline-flex items-center gap-2 rounded-full border border-emerald-100 bg-emerald-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-emerald-700">Tambah Rak</div><button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 transition hover:border-slate-300 hover:text-slate-900" @click="createModalOpen = false"><span class="sr-only">Tutup modal</span><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M6 6l12 12M18 6 6 18" /></svg></button></div>
                    @if ($errors->any() && old('_modal') === 'create')
                        <div class="mt-5 rounded-[1.35rem] border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">Periksa kembali input data rak.</div>
                    @endif
                    <form method="POST" action="{{ route('master-data.rak-obat.store') }}" class="mt-6 space-y-4">
                        @csrf
                        <input type="hidden" name="_modal" value="create">
                        <div class="space-y-2"><label class="text-sm font-semibold text-slate-800" for="create_name">Nama rak</label><input id="create_name" name="name" type="text" value="{{ old('name') }}" placeholder="Contoh: Rak Depan A1" class="ui-control"></div>
                        <div class="space-y-2"><label class="text-sm font-semibold text-slate-800" for="create_storage_location_id">Lokasi induk</label><select id="create_storage_location_id" name="storage_location_id" class="ui-select-control"><option value="">Tanpa lokasi induk</option>@foreach ($locations as $location)<option value="{{ $location->id }}" @selected((string) old('storage_location_id') === (string) $location->id)>{{ $location->name }}</option>@endforeach</select></div>
        ...
        @if ($editingRack)
            <div x-cloak x-show="editModalOpen" x-transition.opacity class="fixed inset-0 z-40 overflow-y-auto bg-slate-950/50 backdrop-blur-sm" @click.self="closeEdit()">
                <div class="flex min-h-full items-start justify-center p-4 sm:items-center sm:p-6">
                <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                    <div class="flex items-start justify-between gap-4"><div class="inline-flex items-center gap-2 rounded-full border border-sky-100 bg-sky-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-sky-700">Ubah Rak</div><button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-500 transition hover:border-slate-300 hover:text-slate-900" @click="closeEdit()"><span class="sr-only">Tutup modal</span><svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round"><path d="M6 6l12 12M18 6 6 18" /></svg></button></div>
                    @if ($errors->any() && old('_modal') === 'edit')
                        <div class="mt-5 rounded-[1.35rem] border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">Periksa kembali input data rak.</div>
                    @endif
                    <form method="POST" action="{{ route('master-data.rak-obat.update', $editingRack) }}" class="mt-6 space-y-4">
                            @csrf @method('PATCH')
                            <input type="hidden" name="_modal" value="edit"><input type="hidden" name="_edit_id" value="{{ $editingRack->id }}">
                            <div class="space-y-2"><label class="text-sm font-semibold text-slate-800" for="edit_name">Nama rak</label><input id="edit_name" name="name" type="text" value="{{ old('name', $editingRack->name) }}" class="ui-control ui-control--sky"></div>
                            <div class="space-y-2"><label class="text-sm font-semibold text-slate-800" for="edit_storage_location_id">Lokasi induk</label><select id="edit_storage_location_id" name="storage_location_id" class="ui-select-control ui-control--sky"><option value="">Tanpa lokasi induk</option>@foreach ($locations as $location)<option value="{{ $location->id }}" @selected((string) old('storage_location_id', $editingRack->storage_location_id) === (string) $location->id)>{{ $location->name }}</option>@endforeach</select></div>
                            <div class="space-y-2"><label class="text-sm font-semibold text-slate-800" for="edit_description">Deskripsi</label><textarea id="edit_description" name="description" rows="3" class="w-full rounded-[1.35rem] border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-900 shadow-sm transition focus:border-sky-300 focus:bg-white focus:outline-none focus:ring-4 focus:ring-sky-100">{{ old('description', $editingRack->description) }}</textarea></div>
                            <label class="flex items-start gap-3 rounded-[1.35rem] border border-slate-200/80 bg-slate-50/80 px-4 py-4"><input type="checkbox" name="is_active" value="1" @checked(old('is_active', $editingRack->is_active)) class="mt-1 h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-200"><span><span class="block text-sm font-semibold text-slate-900">Rak aktif</span><span class="mt-1 block content-copy">Jika aktif, rak siap dipakai sebagai referensi penempatan fisik.</span></span></label>
