<x-app-layout>
    ...
    <div
        x-data="{
            ...
            editModalOpen: @js($editingCategory !== null),
            closeEdit() {
                this.editModalOpen = false;
                window.location = @js(route('master-data.kategori-obat'));
            },
        }"
        @open-create-category-modal.window="createModalOpen = true"
                ...
                <div class="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                    <div class="flex flex-wrap items-center gap-2.5">
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">
                            {{ number_format($stats['total']) }} master
                        </span>
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">
                            {{ number_format($stats['active']) }} aktif
                        </span>
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">
                            {{ number_format($stats['types']) }} tipe
                        </span>
                        <span class="inline-flex items-center rounded-full bg-slate-100 px-4 py-2 text-[0.78rem] font-medium text-slate-700">
                            {{ number_format($stats['categories'] + $stats['groups']) }} kategori & golongan
                        </span>
                    </div>

                    <form method="GET" action="{{ route('master-data.kategori-obat') }}" class="flex flex-wrap items-center justify-end gap-2">
                        <div class="w-[190px] shrink-0">
                        <label for="search" class="sr-only">Cari master</label>
                        <input
                            id="search"
                            name="search"
                            type="text"
                        ...
                        <label for="classification_type" class="sr-only">Tipe</label>
                        <select
                            id="classification_type"
                            name="classification_type"
                            class="ui-select-control"
                        >
                            <option value="all" @selected($classificationType === 'all')>Semua tipe</option>
                            @foreach ($typeOptions as $value => $label)
                                <option value="{{ $value }}" @selected($classificationType === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                        </div>
                                ...
                                <td class="px-4 py-2.5">
                                    <div class="flex min-h-8 items-center">
                                        <span class="inline-flex rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600">
                                            {{ $typeOptions[$item->classification_type] ?? $item->classification_type }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-4 py-2.5">
                                ...
                                <td class="px-4 py-2.5 text-center align-middle">
                                    ...
                                    >
                                        ...
                                        <template x-teleport="body">
                                            ...
                                            >
                                            <a
                                                href="{{ route('master-data.kategori-obat', ['edit' => $item->id]) }}"
                                            ...
                                            <button
                                                ...
                                                @click="close(); openDeleteDialog(@js([
                                                    'action' => route('master-data.kategori-obat.destroy', $item),
                                                    'title' => 'Hapus master kategori obat ini?',
                                                    'description' => 'Master '.$item->name.' akan dihapus dari daftar kategori obat.',
                                                    'warning' => 'Pastikan master ini sudah tidak dipakai pada data obat atau referensi lain.',
                                                    'name' => $item->name,
                                                    'code' => $item->code,
                                                    'confirm_label' => 'Ya, hapus master',
                                                ]))"
                                            >
                                                <svg class="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round">
                                                    <path d="M3 6h18" />
                            ...
                            <tr>
                                <td colspan="6" class="px-5 py-14 text-center">
                                    <div class="mx-auto max-w-md space-y-3">
                                        <div class="empty-title">Belum ada master kategori obat</div>
                                        <p class="content-copy">
                                            Tambah item pertama untuk mulai mengisi jenis, kategori, dan golongan pada form data obat.
                                        </p>
                                        <button
                                            type="button"
                ...
                <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                    ...
                    @if ($errors->any() && old('_modal') === 'create')
                        <div class="mt-5 rounded-[1.35rem] border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
                            Periksa kembali input master kategori obat.
                        </div>
                    @endif

                    <form method="POST" action="{{ route('master-data.kategori-obat.store') }}" class="mt-6 space-y-4">
                        @csrf
                        <input type="hidden" name="_modal" value="create">

                        <div class="space-y-2">
                            <label for="create_classification_type" class="text-sm font-semibold text-slate-800">Tipe master</label>
                            <select
                                id="create_classification_type"
                                name="classification_type"
                                class="ui-select-control"
                            >
                                @foreach ($typeOptions as $value => $label)
                                    <option value="{{ $value }}" @selected(old('classification_type') === $value)>{{ $label }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="space-y-2">
                            ...
                            <textarea
                                ...
                                class="w-full rounded-[1.35rem] border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-emerald-300 focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-100"
                        ...
                        <label class="flex items-start gap-3 rounded-[1.35rem] border border-slate-200/80 bg-slate-50/80 px-4 py-4">
                            ...
                            <span>
                                <span class="block text-sm font-semibold text-slate-900">Master aktif</span>
                                <span class="mt-1 block content-copy">Jika aktif, item langsung muncul di dropdown data obat.</span>
                            </span>
                        </label>
            ...
            >
                <div class="flex min-h-full items-start justify-center p-4 sm:items-center sm:p-6">
                    <div class="panel-surface relative z-50 max-h-[calc(100vh-2rem)] w-full max-w-3xl overflow-y-auto p-6 sm:max-h-[calc(100vh-3rem)] sm:p-7">
                        ...
                        <form method="POST" action="{{ route('master-data.kategori-obat.update', $editingCategory) }}" class="mt-6 space-y-4">
                            @csrf
                            @method('PATCH')
                            <input type="hidden" name="_modal" value="edit">
                            <input type="hidden" name="_edit_id" value="{{ $editingCategory->id }}">

                            <div class="space-y-2">
                                <label for="edit_classification_type" class="text-sm font-semibold text-slate-800">Tipe master</label>
                                <select
                                    id="edit_classification_type"
                                    name="classification_type"
                                    class="ui-select-control ui-control--sky"
                                >
                                    @foreach ($typeOptions as $value => $label)
                                        <option value="{{ $value }}" @selected(old('classification_type', $editingCategory->classification_type) === $value)>{{ $label }}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <input
                                    ...
                                    placeholder="Contoh: Tablet"
                                    class="ui-control ui-control--sky"
                                >
                            </div>
                            ...
                            <div class="space-y-2">
                                ...
                                <textarea
                                    ...
                                    rows="3"
                                    placeholder="Catatan singkat bila perlu"
                                    class="w-full rounded-[1.35rem] border border-slate-200 bg-slate-50 px-4 py-3 text-sm leading-6 text-slate-900 shadow-sm transition placeholder:text-slate-400 focus:border-sky-300 focus:bg-white focus:outline-none focus:ring-4 focus:ring-sky-100"
                                >{{ old('description', $editingCategory->description) }}</textarea>
                            </div>
                            ...
                            <label class="flex items-start gap-3 rounded-[1.35rem] border border-slate-200/80 bg-slate-50/80 px-4 py-4">
                                <input
                                    ...
                                    name="is_active"
                                    value="1"
                                    @checked(old('is_active', $editingCategory->is_active))
